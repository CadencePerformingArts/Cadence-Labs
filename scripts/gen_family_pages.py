...
<script>(function(){{var t=localStorage.getItem("cad-theme");if(t)document.documentElement.dataset.theme=t;
var fs=localStorage.getItem("cad-fontsize");if(fs&&fs!=="1")document.documentElement.style.zoom=fs;}})();</script>
...
<header class="topbar">
  ...
  <nav id="nav">
    <a href="#/" data-route="rankings"><span>{nav[0]}</span></a>
    <a href="#/events" data-route="events"><span>{nav[1]}</span></a>
    <a href="#/corps" data-route="corps"><span>{nav[2]}</span></a>
    <a href="#/data" data-route="data"><span>{nav[3]}</span></a>
  </nav>
...
<div class="demo-strip">{c["strip"]}</div>
...
<footer class="foot">
  ...
  <div>Demo data for product preview. {c["credit"]}</div>
