...
(function () {
  ...
  async function viewEvents(qs, stale) {
    ...
    app.innerHTML = `
      <h1 class="page">${yearPickerHtml(year)} ${FAM ? FAM.eventsTitle : "Shows"} <span class="kicker" id="evCount"></span></h1>
      ${noteHtml("events")}
  ...
  async function viewCaptions(qs, stale) {
    ...
    function renderShowCmp() {
      ...
      body.innerHTML = `<div class="tscroll"><table class="t sticky1 showcmp"><thead><tr><th>${TERM_TH}</th>${HEAD.map(([k, l]) =>
